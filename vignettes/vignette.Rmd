...
export FCS files based on specific cells (saveRDS(fcs_fs, "fcs_fs.rds").
...
Warning: if multiple matrices are present, use the correct one!  
...
The seu_transform function will return a Seurat object with transformed data 
...
# Quality control
(Flow) cytometry is an inherently noisy technology. There will be events close 
to the axis or artefacts due to antibody aggregation. Seumetry provides multiple 
quality control tools to achieve clean data for downstream analysis.

## Removal of outliers
It can occur with (Flow) cytometry, that some events have extremely negative or 
...
plot_cyto(seu, "CD4", "CD3", style = "hex", assay = "comp", slot = "data")
```
...
plot_cyto(seu_manual, "CD4", "CD3", style = "hex", assay = "comp", slot = "data")
```
...
If a channel combination does not look like it contains aggregates, remove them 
from that row from the data.frame (problem_channels[[3]]).  
...
If UMAPs and clustering are not clean, it can help to only select features for 
PCA that distinguish expected cell subsets, e.g. CD3, CD4, and CD8 if working 
with T cells.  
